...
#define CUTOFF_MIN 100.0f
#define CUTOFF_MAX 10000.0f
static float map_frequency_param(float param01) {
   return CUTOFF_MIN * powf(CUTOFF_MAX / CUTOFF_MIN, param01);
}
static float map_frequency_param_inverse(float freq_hz) {
   return logf(freq_hz / CUTOFF_MIN) / logf(CUTOFF_MAX / CUTOFF_MIN);
}
...
static bool my_plug_params_value_to_text(const clap_plugin_t *plugin, clap_id param_id, double value, char *display, uint32_t size) {
   int written;
   if (param_id == 0)
      written = snprintf(display, (size_t)size, "%.1f Hz", (double)map_frequency_param((float)value));
   else
      written = snprintf(display, (size_t)size, "%.2f", value);
...
static bool my_plug_params_text_to_value(const clap_plugin_t *plugin, clap_id param_id, const char *display, double *value) {
   char *end = NULL;
   errno = 0;
...
static void my_plug_params_flush(const clap_plugin_t *plugin, const clap_input_events_t *in, const clap_output_events_t *out) {
   ...
   if (hdr->space_id == CLAP_CORE_EVENT_SPACE_ID) {
      switch (hdr->type) {
        ...
        case CLAP_EVENT_PARAM_MOD: {
            ...
            // Hint: store ev->amount into freq_mod or damping_mod based on ev->param_id
            break;
        }
...
static clap_process_status my_plug_process(const struct clap_plugin *plugin,
   ...
   for (uint32_t ch = 0; ch < nchannels; ++ch) {
      FilterState *state = &plug->filter_state[ch];
