...
static bool my_plug_params_value(const clap_plugin_t *plugin, clap_id param_id, double *value) {
   // @WORKSHOP: connect parameter values to local representation
   return false;
}
...
static bool my_plug_params_value_to_text(const clap_plugin_t *plugin, clap_id param_id, double value, char *display, uint32_t size) {
   int written = snprintf(display, (size_t)size, "%.2f", value);
...
static clap_process_status my_plug_process(const struct clap_plugin *plugin,
   ...
   const clap_audio_buffer_t *input_buffer  = &process->audio_inputs[0];
   ...
   for (uint32_t ch = 0; ch < nchannels; ++ch) {
      for (uint32_t n = 0; n < nframes; ++n) {
         // fetch input samples
         const float x = input_buffer->data32[ch][n];

         // @WORKSHOP: actually do some audio processing here!
         // For now we just pass the input through to the output.
         float y = x * plug->gain;
