...
const PLUGIN_ID: &[u8] = b"org.ccrma.multiband-distortion-plugin\0";
...
static S_MY_PLUG_DESC: clap_plugin_descriptor = clap_plugin_descriptor {
    ...
    name:         b"Multiband Distortion Plugin\0".as_ptr() as *const c_char,
    ...
    description:  b"A multiband distortion plugin.\0".as_ptr() as *const c_char,
...
struct MyPlugin {
    ...
    low_drive_db:      f32, // dB
    high_drive_db:     f32, // dB
    crossover_freq_01: f32,
    output_gain_db:    f32, // dB
...
unsafe extern "C" fn my_plug_params_value_to_text(
    ...
    let s = if param_id == 0 || param_id == 1 || param_id == 3 {
        format!("{:.1} dB", value)
    } else { // param_id == 2: crossover frequency
        format!("{:.1} Hz", map_crossover_param(value as f32))
    };
...
unsafe extern "C" fn my_plug_params_text_to_value(
    ...
    *value = if param_id == 2 {
        map_crossover_param_inverse(v as f32) as f64
    } else {
        v
    };
    true
...
unsafe extern "C" fn my_plug_process(
    ...
    //   let crossover_hz = map_crossover_param(plug.crossover_freq_01);
    //   let low_drive    = db_to_gain(plug.low_drive_db);
    //   let high_drive   = db_to_gain(plug.high_drive_db);
    //   let output_gain  = db_to_gain(plug.output_gain_db);
