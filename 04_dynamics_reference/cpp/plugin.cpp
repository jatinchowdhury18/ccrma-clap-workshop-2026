...
struct My_Plugin : Plugin
{
    ...
    Crossover_Filter_State crossover_state[2];

    float low_band_level_state[2]  = {};
    float high_band_level_state[2] = {};

    // pre-allocated work buffers (sized in activate)
    std::vector<float> low_band;
    std::vector<float> high_band;
    ...
    bool activate(double sampleRate, uint32_t minFrameCount, uint32_t maxFrameCount) noexcept override {
        fs = (float) sampleRate;
        low_band.resize(maxFrameCount);
        high_band.resize(maxFrameCount);
        gain.resize(maxFrameCount);
        return true;
    }
    ...
    void apply_compression(float *data, uint32_t nframes, int ch, bool is_low_band) noexcept
    {
        const float threshold_lin  = std::pow(10.0f, threshold_db  / 20.0f);
        const float makeup_gain_lin = std::pow(10.0f, makeup_gain_db / 20.0f);
        const float knee_lin       = std::pow(10.0f, knee_db        / 20.0f);

        const float half_knee  = std::sqrt(knee_lin);
        const float knee_lower = threshold_lin / half_knee;
        const float knee_upper = threshold_lin * half_knee;
        const float log_thresh = std::log(threshold_lin);
        const float log_knee   = std::log(knee_lin);
        const float rho        = (1.0f / ratio) - 1.0f;

        // compute gain reduction
        for (uint32_t n = 0; n < nframes; ++n)
            gain[n] = std::fabs(data[n]);

        static constexpr float _8x_log10 = (float)(8.0 * 2.30258509299404568);
        for (uint32_t n = 0; n < nframes; ++n)
        {
            const float level = gain[n];
            if (level <= knee_lower)
            {
                gain[n] = 1.0f;
            }
            else if (level >= knee_upper)
            {
                gain[n] = std::pow(level / threshold_lin, rho);
            }
            else
            {
                const float log_x = std::log(level);
                const float root  = log_knee + 2.0f * (log_x - log_thresh);
                gain[n] = std::pow(10.0f, ((1.0f - ratio) * root * root) / (_8x_log10 * ratio * log_knee));
            }
            gain[n] *= makeup_gain_lin;
        }

        // envelope follower (level detector)
        float *z = is_low_band ? &low_band_level_state[ch] : &high_band_level_state[ch];
        const float attack_b0  = 1.0f - std::exp(-1.0f / (fs * attack_ms  * 0.001f));
        const float release_b0 = 1.0f - std::exp(-1.0f / (fs * release_ms * 0.001f));
        for (uint32_t n = 0; n < nframes; ++n)
        {
            const float level = gain[n];
            const float b0    = (level > *z) ? release_b0 : attack_b0;
            *z += b0 * (level - *z);
            gain[n] = *z;
        }
    ...
    clap_process_status process(const clap_process_t *proc) noexcept override {
        ...
        static constexpr float CROSSOVER_MIN = 100.0f;
        static constexpr float CROSSOVER_MAX = 10000.0f;
        const float crossover_hz = CROSSOVER_MIN * std::pow(CROSSOVER_MAX / CROSSOVER_MIN, crossover_freq_01);
        ...
        {
            // Split into low and high bands via SVF crossover
            Crossover_Filter_State &s = crossover_state[ch];
            for (uint32_t n = 0; n < nframes; ++n)
            {
                const float input = proc->audio_inputs[0].data32[ch][n];
                const float t0 = input - s.stage1_ic2eq;
                const float v0 = gt0 * t0 - gk0 * s.stage1_ic1eq;
                const float t1 = gt1 * t0 - gk1 * s.stage1_ic1eq;
                const float t2 = gt2 * t0 + gt1 * s.stage1_ic1eq;
                const float v1 = t1 + s.stage1_ic1eq;
                const float v2 = t2 + s.stage1_ic2eq;
                s.stage1_ic1eq += 2.0f * t1;
                s.stage1_ic2eq += 2.0f * t2;
                low_band[n]  = v2;
                high_band[n] = v0;
            }

            apply_compression(low_band.data(),  nframes, (int)ch, true);
            apply_compression(high_band.data(), nframes, (int)ch, false);
